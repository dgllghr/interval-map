...
module Make (Bound : Bound.S) = struct
  ...
  let is_valid_bounds low high =
    match (low, high) with
    | Included low, Included high -> C.compare low high <= 0
  ...
  let create low high =
    if is_valid_bounds low high then { low; high } else raise Invalid_interval
  ...
  let compare a b =
    ...
    if low_bound_cmp = 0 then high_bound_compare a b else low_bound_cmp

  let overlap_low_high ivl_a ivl_b =
    let low =
      if low_bound_compare ivl_a ivl_b < 0 then ivl_b.low else ivl_a.low
    in
    let high =
      if high_bound_compare ivl_a ivl_b < 0 then ivl_a.high else ivl_b.high
    in
    (low, high)
  ...
  let overlap_interval ivl_a ivl_b =
    ...
    if is_valid interval then Some interval else None
