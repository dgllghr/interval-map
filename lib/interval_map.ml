...
module Make (Bound_compare : Comparable) = struct
  ...
  module Tree = struct
    ...
    let max_bound (interval : Interval.t) left right =
      ...
      | None, None -> mid
      | None, Some r -> Bound.max_upper mid r.max
      | Some l, None -> Bound.max_upper mid l.max
      | Some l, Some r -> Bound.max_upper mid (Bound.max_upper l.max r.max)

    let min_bound (interval : Interval.t) left right =
      let mid = interval.low in
      match (left, right) with
      | None, None -> mid
      | None, Some r -> Bound.min_lower mid r.min
      | Some l, None -> Bound.min_lower mid l.min
      | Some l, Some r -> Bound.min_lower mid (Bound.min_lower l.min r.min)
    ...
    let size = function
      ...
      | Some node ->
          let rec go { left; right; values; _ } size =
            ...
            match (left, right) with
            | None, None -> size
            | Some lt, None -> go lt size
            | None, Some rt -> go rt size
            | Some lf, Some rt ->
    ...
    let rec find interval = function
      ...
      | Some node ->
          ...
          if ivl_cmp = 0 then Some node
          else if ivl_cmp < 0 then find interval node.left
          else find interval node.right
          ...
          let res =
            if ivl_cmp < 0 then
              ...
              let insert_right =
                ...
                | None -> leaf interval [ value ]
                | rt -> add interval value rt
              in
              replace_right node insert_right
            else { node with values = value :: node.values }
          in
          Some (balance res)

    let rec add_replace_all interval values = function
      | None -> leaf interval values
      | Some node ->
          let ivl_cmp = Interval.compare interval node.interval in
          let res =
            if ivl_cmp < 0 then
              let insert_left =
                match node.left with
                | None -> leaf interval values
                | lt -> add_replace_all interval values lt
              in
              replace_left node insert_left
            else if ivl_cmp > 0 then
              let insert_right =
                match node.right with
                | None -> leaf interval values
                | rt -> add_replace_all interval values rt
              in
              replace_right node insert_right
            else { node with values }
          in
          Some (balance res)
          ...
          let rec remove node =
            ...
            let res =
              if ivl_cmp = 0 then
                ...
                | _ :: _, _, _ -> Some { node with values }
                | _, None, None -> None
                | _, Some lt, None -> Some lt
                | _, None, Some rt -> Some rt
                | _, Some _, Some rt ->
  ...
  module Gen = struct
    ...
    let filter_option fn = function
      | Some v as opt -> if fn v then opt else None
      | None -> None
    ...
    let rec next { node; stack; order; go_left; go_right; emit } =
      ...
      | Some node, stack ->
          ...
          let next_node =
            match order with
            | Asc -> filter_option go_left node.left
            | Desc -> filter_option go_right node.right
          in
          next { node = next_node; stack; order; go_left; go_right; emit }
      | None, node :: stack ->
          let next_node =
            match order with
            | Asc -> filter_option go_right node.right
            | Desc -> filter_option go_left node.left
          in
          if emit node then
            Some
              ( (node.interval, node.values),
                { node = next_node; stack; order; go_left; go_right; emit } )
          else next { node = next_node; stack; order; go_left; go_right; emit }
      | None, [] -> None
  ...
  let query_interval ?(order = Asc) query =
    ...
    let go_left left =
      ...
      | Included max, Included low -> Bound.C.compare max low >= 0
    ...
    let go_right right =
      ...
      | Included min, Included high -> Bound.C.compare min high <= 0
  ...
  let mapi fn =
    Tree.fold_bf
      (fun map node ->
        Tree.add_replace_all node.interval
